...
valid_ind <- data_list$valid_ind
train_ind <- (1:length(train_y)) %>% setdiff(valid_ind)
...
if (FEAT_SEL) {
    ...
    write_rds(list(rm_feats = rm_feats, valid_f1 = perf_track), str_c(processed_data_dir, "rm_feats.rds"))
}

rm_feats <- read_rds(str_c(processed_data_dir, "rm_feats.rds"))
...
tune_result_file_name <- str_c("xgb",
                               ifelse(USE_PCA, "_PCA", ""),
                               "_tune_result.rds")
...
# write_rds(mod, str_c(result_dir, "trained_xgb",
#                       ifelse(USE_PCA, "_PCA", ""),
#                       "_model.rds"))
print("single xgb model finished training.")
...
subm <- read_csv(paste0(data_dir, "submission_test.csv"),
                 col_types = cols(txkey = col_character()))
subm$fraud_ind <- (pred_test$data %>% pull(response))
write_csv(subm, paste0(result_dir, "my_xgb",
                       ifelse(USE_PCA, "_PCA", ""),
                       "_submission.csv"))
